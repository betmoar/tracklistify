...
from tracklistify.rate_limiter import (
    RateLimiter, CircuitState, RateLimitMetrics, ProviderLimits
)
...
class TestRateLimiter:
    ...
    async def test_metrics_tracking(self, rate_limiter, mock_provider):
        ...
        assert metrics["rate_limited_requests"] == 0  # Only counts requests that wait and succeed
    ...
    def test_provider_registration(self, rate_limiter, mock_provider):
        ...
        assert limits.tokens == 30  # Initial tokens should match max_requests_per_minute
    ...
    async def test_rate_limit_windows(self, rate_limiter, mock_provider):
        ...
        try:
            ...
            assert window[1] - window[0] < 1.0  # Window should be short since we used small timeout
        finally:
            # Ensure cleanup
            rate_limiter.release(mock_provider)
