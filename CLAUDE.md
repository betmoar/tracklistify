...
- **Dedup lives in `get_unique_tracks` only** (`add_track` just gates+appends). Cluster proximity is anchored on `cluster[0]`, never "any member" — the any-member form chains transitively and silently deletes distinct plays. `_rep_key` must never read confidence (Shazam jitter → unstable output). Before touching any of it, read `docs/playbooks/changing-dedup.md`.
