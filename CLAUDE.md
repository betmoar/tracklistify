...
- **`--no-cache` is a REFRESH, not a disable.** It sets `config.cache_refresh`, which skips both cache *reads* while leaving both *writes* live, so a wrong stored identification is overwritten. Setting `cache_enabled = False` instead would be a bug: both write paths gate on the same enabled flag, so the stale entry would survive on disk and be served again on the next normal run. Entries are content-hash keyed with a 30-day TTL, so a bad match otherwise reproduces on every re-run.
...
- **Dedup lives in `get_unique_tracks` only** (`add_track` just gates+appends). The two cluster axes anchor on opposite ends and neither may be "any member" — that form chains transitively and silently deletes distinct plays. **Proximity** is anchored on `cluster[-1]` (the most recent detection), so an unbroken step cadence can extend past the window; anchoring it on `cluster[0]` bounds the span and splits long tracks. **Identity** is anchored on `cluster[0]`, so membership cannot drift — artist Jaccard is not transitive, and a collaboration credit will otherwise bridge two distinct artists. `_rep_key` must never read confidence (Shazam jitter → unstable output). `_tracks_match` compares titles after `_strip_title_variant` rewrites trailing-suffix bracket groups (comparison-only; drops non-distinguishing version/live tags like `(Club Mix)`/`[Live At …]`; canonicalizes `feat.`/`ft.`/`featuring` to `feat` keeping word + credited artist; default keep, so `remix`/`bootleg`/`edit by`/`vip` (whole words), leading/middle brackets, and the representative's raw `song_name` stay untouched). Before touching any of it, read `docs/playbooks/changing-dedup.md`.
...
- **An invalid `.github/dependabot.yml` disables Dependabot entirely**, not just the bad entry, and GitHub reports it through a *check run* rather than the Actions log — so updates stop silently. Only ONE entry per `(package-ecosystem, directory, target-branch)`; per-dependency treatment goes in `groups` (unlimited), never a second entry. Valid YAML is not valid config: run `uv run python scripts/check_dependabot_config.py` (wired into the CI `drift` job).
- **A stale `yt-dlp` pin is a total YouTube outage, not a missing feature.** YouTube retires the player clients an older version reaches for; the symptom is `HTTP Error 403: Forbidden` on *every* download while metadata extraction still succeeds (so anything that stops at `extract_info` reports green). A `>=` floor in `pyproject.toml` does not help — only `uv lock --upgrade-package yt-dlp` does. Measured: `2026.7.4` 403'd where `2026.8.19` downloaded (#91). Probe video must be one that actually fails: short/old videos (e.g. `jNQXAC9IVRw`) download fine on broken versions.
