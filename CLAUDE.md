...
- **`config.min_confidence` (0–1) and `Track.confidence` (0–100) are different scales**: the config knob is now wired (`TrackMatcher.__init__` scales it `* 100`). Default is `0.0` (keep all tracks); raising it drops sub-threshold matches. The `mock_config` fixture must stay at `0.0` or it silently filters the whole test suite.
