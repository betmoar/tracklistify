...
from .base import MetadataProvider, ProviderError, TrackIdentificationProvider
...
class ProviderFactory:
    ...
    def __init__(self):
        """Initialize the ProviderFactory with empty provider dictionaries."""
        self._identification_providers: Dict[
            str, TrackIdentificationProvider
        ] = {}
    ...
    def register_identification_provider(
        ...
        if not isinstance(provider, TrackIdentificationProvider):
            raise ValueError(
                f"Provider {name} must implement TrackIdentificationProvider"
            )
    ...
    def get_identification_provider(
        self, name: str
    ) -> Optional[TrackIdentificationProvider]:
    ...
    def get_all_identification_providers(
        self,
    ) -> List[TrackIdentificationProvider]:
...
def create_provider_factory(
    ...
    try:
        ...
        if not factory.get_all_identification_providers():
            raise ProviderError(
                "No identification providers configured. Please check your configuration."
            )
