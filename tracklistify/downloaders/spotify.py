...
class SpotifyDownloader(Downloader):
    ...
    def __init__(self, cookies_path: Optional[str] = None, verbose: bool = False,
        ...
        # Setup directories
        self.output_dir = Path(output_dir if output_dir else DEFAULT_OUTPUT_DIR).expanduser()
        ...
        logger.debug(f"Initialized SpotifyDownloader with ffmpeg at: {self.ffmpeg_path}")
    ...
    def _set_metadata(self, file_path: str, metadata: Dict[str, Any]):
        ...
        for key, tag in tags.items():
            if key == "artist":
                ...
                value = (metadata["disc_number"], metadata["album"].get("total_discs", 1))
            elif key == "genre":
    ...
    async def download(self, url: str) -> Optional[str]:
        ...
        if not self._cookies:
            raise DownloadError("Spotify cookies required. Please provide cookies_path.")
        ...
        try:
            ...
            if self.format != AudioFormat.OGG:
                cmd = [
                    self.ffmpeg_path,
                    "-i", str(temp_path),
                    "-c:a", "aac" if self.format == AudioFormat.M4A else "libmp3lame",
                    "-b:a", self.quality.value.split("_")[-1] + "k",
                    "-metadata", f"title={metadata['name']}",
                    "-metadata", f"artist={metadata['artists'][0]['name']}",
                    "-y", str(output_path)
                ]
            ...
            logger.info(f"Downloaded: {metadata['name']} by {metadata['artists'][0]['name']}")
