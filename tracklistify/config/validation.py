...
class RangeRule(ValidationRule):
    ...
    def __init__(
        ...
        self.description = f"must be positive" if min_value == 0 else f"must be between {min_value} and {max_value}"
...
def validate_field_type(value: Any, field_type: Type[T]) -> T:
    ...
    if not isinstance(value, field_type):
        raise TypeError(f"Expected type {field_type.__name__}, got {type(value).__name__}")
    return value

def validate_path(value: Union[str, Path], field_name: str = 'path', must_exist: bool = True) -> Path:
    """
...
def validate_config_dict(config: Dict[str, Any], config_class: Type[T]) -> Dict[str, Any]:
