...
from ..exceptions import ExportError, AuthenticationError
...
class SpotifyPlaylistExporter:
    ...
    async def export_playlist(self, tracks: List[Track], playlist_name: Optional[str] = None) -> str:
        """
    ...
    async def _create_playlist(self, name: str) -> str:
        ...
        data = {
            "name": name,
            "description": "Created by Tracklistify",
            "public": True
        }
    ...
    async def _add_tracks_to_playlist(self, playlist_id: str, track_ids: List[str]):
        ...
        for i in range(0, len(track_ids), 100):
            ...
            async with self.spotify._session.post(
                ...
                if response.status != 201:
                    raise ExportError(f"Failed to add tracks to playlist: {response.status}")